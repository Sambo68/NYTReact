...
const loadArticleSeed = () => {
  const apiArticles = JSON.parse(fs.readFileSync('sample-article-results.json', 'utf8'));
  return apiArticles.response.docs.map(article => {
    return {
      url: article.web_url,
...
const articleSeed = loadArticleSeed();
db.Article
  .remove({})
